...
struct IndexEntry<'a> {
    ...
    offset: usize,
    length: usize,
    raw_utc_offset: usize,
}

pub(super) fn find_file() -> Result<File, Error> {
    for (env_var, dir) in
...
pub(super) fn find_tz_data_in_file(file: &mut File, tz_name: &str) -> Result<Vec<u8>, Error> {
...
impl Header {
    fn new(file: &mut File) -> Result<Self, Error> {
        ...
        Ok(Self {
            ...
            zonetab_offset: parse_tz_int(&buf, MAGIC_SIZE + 2 * TZ_INT_SIZE) as usize,
        })
    }
}
...
impl<'a> IndexEntry<'a> {
    fn new(bytes: &'a [u8]) -> Self {
        Self {
            name: bytes[..TZ_NAME_SIZE].splitn(2, |&b| b == 0u8).next().unwrap(),
            offset: parse_tz_int(bytes, TZ_NAME_SIZE) as usize,
            length: parse_tz_int(bytes, TZ_NAME_SIZE + TZ_INT_SIZE) as usize,
            raw_utc_offset: parse_tz_int(bytes, TZ_NAME_SIZE + 2 * TZ_INT_SIZE) as usize,
        }
    }
}
